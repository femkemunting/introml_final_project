...
train_indices <- sample(1:nrow(filtered_spotify), nrow(filtered_spotify) / 2)
training_set <- filtered_spotify[train_indices, ]

kcv = 10

cv_folds = createFolds(spot_train$popularity,
                               k = kcv)
...
tree_model <- rpart(formula = popularity ~ danceability + energy + loudness,
                    data = filtered_spotify,
